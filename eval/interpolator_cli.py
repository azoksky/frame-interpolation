...
def _process_directory(directory: str):
    """Process one directory of frames: perform interpolation on separate segments."""
    ...
    if num_gpus >= 2:
        # Split non-overlapping: for even n, GPU0 gets first half; for odd n, GPU0 gets the extra frame.
                ...
                with tqdm(total=total_steps, desc=f"GPU {gpu_index} Interpolation", ncols=100) as pbar:
                    for i in range(1, len(seg_frames)):
                        pair_frames = list(util._recursive_generator(
                            ...
                            device_str,
                            pbar=pbar))
                        # For each adjacent pair in the segment, include all but the last frame
                        # to avoid duplicates (except for the final pair).
        ...
        # Simply merge the segments without any additional boundary interpolation.
        combined_frames = results[0] + results[1]
