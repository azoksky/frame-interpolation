...
def _recursive_generator_chain(
    ...
    """
    Processes a list of frame file paths sequentially using the shared progress bar.
    """
    n = len(frames)
    for i in range(1, n):
...
def interpolate_recursively_from_files(
    ...
    If a shared progress bar (pbar) is provided, it is used exclusively.
    """
    n = len(frames)
    ...
    progress = {"count": 0}
    if pbar is None:
        with tqdm(total=total, desc="Interpolation progress", ncols=100) as local_pbar:
            for i in range(1, n):
                yield from _recursive_generator(
                    read_image(frames[i - 1]),
                    read_image(frames[i]),
